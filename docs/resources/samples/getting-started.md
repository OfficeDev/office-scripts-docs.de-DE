...
### <a name="access"></a><span data-ttu-id="dbf2a-108">Access</span><span class="sxs-lookup"><span data-stu-id="dbf2a-108">Access</span></span>
...
* [<span data-ttu-id="dbf2a-124">macOS</span><span class="sxs-lookup"><span data-stu-id="dbf2a-124">macOS</span></span>](https://code.visualstudio.com/shortcuts/keyboard-shortcuts-macos.pdf)
...
## <a name="gentle-introduction-to-scripting"></a><span data-ttu-id="dbf2a-143">Behutsame Einführung in skripterstellung</span><span class="sxs-lookup"><span data-stu-id="dbf2a-143">Gentle introduction to scripting</span></span>
...
<span data-ttu-id="dbf2a-181">Zusammenfassung:</span><span class="sxs-lookup"><span data-stu-id="dbf2a-181">So, in summary:</span></span>
...
<span data-ttu-id="dbf2a-195">Sie können Funktionen verwenden, um Code innerhalb Ihres Skripts zu organisieren und wiederzuverwenden.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-195">You can use functions to organize and reuse code within your script.</span></span>
...
<span data-ttu-id="dbf2a-226">Sehen wir uns einige Methoden für das Objekt `workbook` an.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-226">Let's look at a few methods on the `workbook` object.</span></span>
...
<span data-ttu-id="dbf2a-235">In diesem Beispiel:</span><span class="sxs-lookup"><span data-stu-id="dbf2a-235">In this example:</span></span>
...
#### <a name="method-chaining"></a><span data-ttu-id="dbf2a-250">Methodenkette</span><span class="sxs-lookup"><span data-stu-id="dbf2a-250">Method chaining</span></span>
...
<span data-ttu-id="dbf2a-271">Verwenden Sie alternativ zum Löschen eines Objekts, das möglicherweise vorhanden ist oder nicht, das folgende Muster.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-271">Alternatively, for deleting an object that may or may not exist, use the following pattern.</span></span>
...
* [<span data-ttu-id="dbf2a-282">`for` oder `while`</span><span class="sxs-lookup"><span data-stu-id="dbf2a-282">`for` or `while`</span></span>](https://developer.mozilla.org/docs/Web/JavaScript/Guide/Loops_and_iteration)
...
<span data-ttu-id="dbf2a-290">Beispiele:</span><span class="sxs-lookup"><span data-stu-id="dbf2a-290">Examples:</span></span>
...
<span data-ttu-id="dbf2a-306">**_Systemeigene Sprachtypen_**</span><span class="sxs-lookup"><span data-stu-id="dbf2a-306">**_Native language types_**</span></span>
...
<span data-ttu-id="dbf2a-354">Der Fehler "Office-Skripts" kann in eine der folgenden Kategorien unterteilt werden.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-354">Office Scripts error can be classified into one of the following categories.</span></span>
...
<span data-ttu-id="dbf2a-371">Verwenden Sie alternativ zum Löschen eines Objekts, das möglicherweise vorhanden ist, dieses Muster:</span><span class="sxs-lookup"><span data-stu-id="dbf2a-371">Alternatively, for deleting an object that may or may not exist, use this pattern:</span></span>
...
<span data-ttu-id="dbf2a-392">Die [`try..catch`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/try...catch) Technik ist eine Möglichkeit, um zu erkennen, ob ein API-Aufruf fehlgeschlagen ist, und diesen Fehler in Ihrem Skript zu behandeln.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-392">The [`try..catch`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/try...catch) technique is a way to detect if an API call failed and handle that error in your script.</span></span> <span data-ttu-id="dbf2a-393">Es kann wichtig sein, den Rückgabewert einer API zu überprüfen, um sicherzustellen, dass sie erfolgreich abgeschlossen wurde.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-393">It may be important to check the return value of an API to verify that it was completed successfully.</span></span>

<span data-ttu-id="dbf2a-394">Betrachten Sie den folgenden Beispielausschnitt.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-394">Consider the following example snippet.</span></span>
...
<span data-ttu-id="dbf2a-395">Der `setValues()` Aufruf kann fehlschlagen und zu einem Skriptfehler führen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-395">The `setValues()` call may fail and result in the script failure.</span></span> <span data-ttu-id="dbf2a-396">Sie können diese Bedingung in Ihrem Code behandeln und möglicherweise die Fehlermeldung anpassen oder das Update in kleinere Einheiten aufbrechen usw. In diesem Fall ist es wichtig zu wissen, dass die API einen Fehler zurückgegeben hat, und diesen Fehler interpretieren oder behandeln.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-396">You may wish to handle this condition in your code and perhaps customize the error message or break up the update into smaller units, etc. In that case, it's important to know that the API returned an error and interpret or handle that error.</span></span>
...
<span data-ttu-id="dbf2a-397">Ein weiteres Szenario ist, wenn die Hauptfunktion eine andere Funktion aufruft, die wiederum eine andere Funktion aufruft (und so weiter).) und der für Sie wichtige API-Aufruf in der unteren Funktion nach unten erfolgt.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-397">Another scenario is when main function calls another function, which in turn calls another function (and so on..), and the API call that you care about happens down in the bottom function.</span></span> <span data-ttu-id="dbf2a-398">Das Weitervermehren des Fehlers bis zu ist möglicherweise nicht `main` machbar oder praktisch.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-398">Propagating the error all the way up to `main` may not be feasible or convenient.</span></span> <span data-ttu-id="dbf2a-399">In diesem Fall ist das Auswerfen eines Fehlers in der unteren Funktion am bequemsten.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-399">In that case, throwing an error in the bottom function will be most convenient.</span></span>
...
<span data-ttu-id="dbf2a-400">*Warnung:* Die `try..catch` Verwendung innerhalb einer Schleife verlangsamt ihr Skript.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-400">*Warning*: Using `try..catch` inside of a loop will slow down your script.</span></span> <span data-ttu-id="dbf2a-401">Vermeiden Sie dies innerhalb oder um Schleifen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-401">Avoid using this inside of or around loops.</span></span>

## <a name="range-basics"></a><span data-ttu-id="dbf2a-402">Grundlagen des Bereichs</span><span class="sxs-lookup"><span data-stu-id="dbf2a-402">Range basics</span></span>

<span data-ttu-id="dbf2a-403">Weitere Informationen finden Sie unter [Range Basics.](range-basics.md)</span><span class="sxs-lookup"><span data-stu-id="dbf2a-403">Check out [Range Basics](range-basics.md) before you go further on your journey.</span></span>

## <a name="basic-performance-considerations"></a><span data-ttu-id="dbf2a-404">Grundlegende Leistungsüberlegungen</span><span class="sxs-lookup"><span data-stu-id="dbf2a-404">Basic performance considerations</span></span>

### <a name="avoid-slow-operations-in-the-loop"></a><span data-ttu-id="dbf2a-405">Vermeiden langsamer Vorgänge in der Schleife</span><span class="sxs-lookup"><span data-stu-id="dbf2a-405">Avoid slow operations in the loop</span></span>

<span data-ttu-id="dbf2a-406">Bestimmte Vorgänge, die innerhalb/um die Loop-Anweisungen wie , , , usw. durchgeführt werden, können zu `for` `for..of` `map` `forEach` leistungsschwenkung führen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-406">Certain operations when done inside/around the loop statements such as `for`, `for..of`, `map`, `forEach`, etc. can lead to slow performance.</span></span> <span data-ttu-id="dbf2a-407">Vermeiden Sie die folgenden API-Kategorien.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-407">Avoid the following API categories.</span></span>

* <span data-ttu-id="dbf2a-408">`get*` APIs</span><span class="sxs-lookup"><span data-stu-id="dbf2a-408">`get*` APIs</span></span>

<span data-ttu-id="dbf2a-409">Lesen Sie alle Daten, die Sie außerhalb der Schleife benötigen, anstatt sie innerhalb der Schleife zu lesen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-409">Read all the data you need outside of the loop rather than reading it inside of the loop.</span></span> <span data-ttu-id="dbf2a-410">Manchmal ist es schwierig, das Lesen innerhalb von Schleifen zu vermeiden. Stellen Sie in diesem Fall sicher, dass die Schleifenanzahl nicht zu groß ist, oder verwalten Sie sie in Batches, um eine Schleife durch eine große Datenstruktur zu vermeiden.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-410">At times, it is hard to avoid reading inside of loops; in such a case, make sure your loop counts are not too large or manage them in batches to avoid having to loop through a large data structure.</span></span>

<span data-ttu-id="dbf2a-411">**Hinweis:** Wenn der Bereich/die Daten, mit dem Sie zu tun haben, recht groß ist (z. B. >100K-Zellen), müssen Sie möglicherweise erweiterte Techniken wie das Aufbrechen von Lese-/Schreibvorgängen in mehrere Blöcke verwenden.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-411">**Note**: If the range/data you are dealing with is quite large (say >100K cells), you may need to use advanced techniques like breaking up your read/writes into multiple chunks.</span></span> <span data-ttu-id="dbf2a-412">Das folgende Video ist wirklich für eine Dateneinrichtung mittlerer Größe.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-412">The following video is really for a small-mid sized data setup.</span></span> <span data-ttu-id="dbf2a-413">Ein großes Dataset finden Sie unter [Advanced Data Write Scenario](write-large-dataset.md).</span><span class="sxs-lookup"><span data-stu-id="dbf2a-413">For a large dataset, refer to [advanced data write scenario](write-large-dataset.md).</span></span>

<span data-ttu-id="dbf2a-414">[![Video, das einen Tipp zur Lese- und Schreiboptimierung bietet](../../images/getting-started-v_perf.jpg)](https://youtu.be/lsR_GvVW3Pg "Video mit Lese- und Schreiboptimierungstipp")</span><span class="sxs-lookup"><span data-stu-id="dbf2a-414">[![Video providing a read-and-write optimization tip](../../images/getting-started-v_perf.jpg)](https://youtu.be/lsR_GvVW3Pg "Video showing read-and-write optimization tip")</span></span>

* <span data-ttu-id="dbf2a-415">`console.log` -Anweisung (siehe das folgende Beispiel)</span><span class="sxs-lookup"><span data-stu-id="dbf2a-415">`console.log` statement (see the following example)</span></span>
...
* <span data-ttu-id="dbf2a-416">`try {} catch ()` -Anweisung</span><span class="sxs-lookup"><span data-stu-id="dbf2a-416">`try {} catch ()` statement</span></span>

<span data-ttu-id="dbf2a-417">Vermeiden Sie `for` Ausnahmebehandlungsschleifen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-417">Avoid exception handling `for` loops.</span></span> <span data-ttu-id="dbf2a-418">Sowohl innerhalb als auch außerhalb von Schleifen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-418">Both inside and outside loops.</span></span>

## <a name="note-to-vba-developers"></a><span data-ttu-id="dbf2a-419">Hinweis für VBA-Entwickler</span><span class="sxs-lookup"><span data-stu-id="dbf2a-419">Note to VBA developers</span></span>

<span data-ttu-id="dbf2a-420">Die TypeScript-Sprache unterscheidet sich sowohl syntaktisch als auch in Benennungskonventionen von VBA.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-420">The TypeScript language differs from VBA both syntactically as well as in naming conventions.</span></span>

<span data-ttu-id="dbf2a-421">Sehen Sie sich die folgenden äquivalenten Codeausschnitte an.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-421">Check out the following equivalent snippets.</span></span>
...
<span data-ttu-id="dbf2a-422">Einige Dinge, die Sie über TypeScript herausrufen müssen:</span><span class="sxs-lookup"><span data-stu-id="dbf2a-422">A few things to call out about TypeScript:</span></span>

* <span data-ttu-id="dbf2a-423">Möglicherweise müssen für alle Methoden offene Klammern zum Ausführen verwendet werden.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-423">You may notice that all methods need to have open-close parentheses to execute.</span></span> <span data-ttu-id="dbf2a-424">Argumente werden identisch übergeben, aber einige Argumente sind möglicherweise für die Ausführung erforderlich (d. h. erforderlich im Vergleich zu optional).</span><span class="sxs-lookup"><span data-stu-id="dbf2a-424">Arguments are passed identically but some arguments may be required for execution (that is, required vs optional).</span></span>
* <span data-ttu-id="dbf2a-425">Die Benennungskonvention folgt camelCase anstelle der PascalCase-Konvention.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-425">The naming convention follows camelCase instead of PascalCase convention.</span></span>
* <span data-ttu-id="dbf2a-426">Methoden weisen in `get` der Regel Präfixe auf, die angeben, ob Objektm members gelesen `set` oder geschrieben werden.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-426">Methods usually have `get` or `set` prefixes indicating whether it is reading or writing object members.</span></span>
* <span data-ttu-id="dbf2a-427">Die Codeblöcke werden durch geschweifte geschweifte Geschweifte mit offenem Schließen definiert und identifiziert: `{` `}` .</span><span class="sxs-lookup"><span data-stu-id="dbf2a-427">The code blocks are defined and identified by open-close curly braces: `{` `}`.</span></span> <span data-ttu-id="dbf2a-428">Blöcke sind für `if` Bedingungen, `while` Anweisungen, `for` Schleifen, Funktionsdefinitionen usw. erforderlich.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-428">Blocks are required for `if` conditions, `while` statements, `for` loops, function definitions, etc.</span></span>
* <span data-ttu-id="dbf2a-429">Funktionen können andere Funktionen aufrufen, und Sie können sogar Funktionen innerhalb einer Funktion definieren.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-429">Functions can call other functions and you can even define functions within a function.</span></span>

<span data-ttu-id="dbf2a-430">Insgesamt ist TypeScript eine andere Sprache, und es gibt nur wenige Ähnlichkeiten zwischen ihnen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-430">Overall, TypeScript is a different language and there are few similarities between them.</span></span> <span data-ttu-id="dbf2a-431">Die Office Scripts-API selbst verwendet jedoch ähnliche Terminologie- und Datenmodellhierarchien (Objektmodell) wie VBA-APIs, und dies sollte Ihnen beim Navigieren helfen.</span><span class="sxs-lookup"><span data-stu-id="dbf2a-431">However, the Office Scripts API themselves use similar terminology and data-model (object model) hierarchy as VBA APIs and that should help you navigate around.</span></span>
